...
def test_selection_detects_saturated_annual_metrics_only() -> None:
    contract = FigeMetricSelectionContract.from_yaml(CONTRACT_PATH)
    report = evaluate_fige_metric_selection(_audit_payload(), contract)
    evidence = {metric.name: metric for metric in report.metrics}

    assert evidence["intermediation_expense_to_revenue"].saturation_evaluated is True
    assert evidence["intermediation_expense_to_revenue"].empirically_saturated is True
    assert (
        evidence["non_continuing_result_gap_to_abs_net_income"].empirically_saturated
        is True
    )
    assert evidence["roa_closing_assets"].empirically_saturated is False

    historical = evidence["net_income_coefficient_of_variation"]
    assert historical.saturation_evaluated is False
    assert historical.empirically_saturated is False

    saturated_warning = next(
        warning
        for warning in report.warnings
        if warning.startswith("EMPIRICALLY_SATURATED_ANNUAL_METRICS:")
    )
    assert "intermediation_expense_to_revenue" in saturated_warning
    assert "non_continuing_result_gap_to_abs_net_income" in saturated_warning
    assert "net_income_coefficient_of_variation" not in saturated_warning
...
def test_selection_fails_closed_on_missing_required_metric_evidence() -> None:
    ...
    economic_audit = payload["economic_audit"]
    assert isinstance(economic_audit, dict)
    annual = economic_audit["annual_audits"]
    assert isinstance(annual, list)
    metrics = annual[0]["metrics"]
    assert isinstance(metrics, dict)
    metrics["roa_closing_assets"] = None
    contract = FigeMetricSelectionContract.from_yaml(CONTRACT_PATH)
...
def test_selection_fails_closed_on_wrong_company_or_year_window() -> None:
    ...
    annual = economic_audit["annual_audits"]
    assert isinstance(annual, list)
    economic_audit["annual_audits"] = annual[1:]
