...
from ultimate_stock_analyzer.backtesting import (
    bank_field_evidence_routing,
    cvm_bank_net_income_canonical_mapping_audit,
    cvm_ipe_pillar3_numeric_values,
)
from ultimate_stock_analyzer.domain import master
...
CVM_SCOPE_BLOCKER = (
    cvm_bank_net_income_canonical_mapping_audit.CVM_BANK_DFP_PRUDENTIAL_SCOPE_ALIGNMENT_UNPROVEN
)
CVM_REVISION_BLOCKER = (
    cvm_bank_net_income_canonical_mapping_audit.CVM_BANK_DFP_REVISION_HISTORY_COMPLETENESS_UNPROVEN
)
PILLAR3_REVISION_BLOCKER = (
    cvm_ipe_pillar3_numeric_values.PILLAR3_IPE_REVISION_HISTORY_COMPLETENESS_UNPROVEN
)
...
) -> master.BankPrudentialAnnualRecord:
    return master.BankPrudentialAnnualRecord(
...
def _cvm_line(
    ...
    *,
    ...
    version: int = 1,
) -> master.FinancialStatementLine:
    return master.FinancialStatementLine(
        company_id="cvm:19348",
        cvm_code=19348,
        cnpj="60.872.504/0001-23",
...
def _cvm_net_income_lines(
    *,
    available_from: datetime = CVM_AVAILABLE_FROM,
) -> list[master.FinancialStatementLine]:
    return [
        _cvm_line(
            "3.07",
...
def _pillar3() -> cvm_ipe_pillar3_numeric_values.Pillar3PrudentialObservation:
    return cvm_ipe_pillar3_numeric_values.Pillar3PrudentialObservation(
...
def test_ifdata_values_are_visible_but_not_strict_pit() -> None:
    report = bank_field_evidence_routing.route_bank_field_evidence(
        _profile(),
...
def test_cvm_309_routes_as_official_scope_mismatch_when_ifdata_is_missing() -> None:
    ...
    assert annual.contract_admissible is False
    assert CVM_SCOPE_BLOCKER in annual.blockers
    assert CVM_REVISION_BLOCKER in annual.blockers
    assert report.observed_critical_coverage == pytest.approx(1.0)
...
def test_as_of_excludes_future_cvm_filing() -> None:
    ...
    assert bank_field_evidence_routing.BANK_FIELD_EVIDENCE_MISSING in annual.blockers
...
def test_as_of_excludes_future_ifdata_estimate() -> None:
    ...
    assert bank_field_evidence_routing.BANK_FIELD_NOT_YET_AVAILABLE_AS_OF in assets.blockers
...
def test_pillar3_routes_prudential_ratios_without_readiness_promotion() -> None:
    ...
    assert PILLAR3_REVISION_BLOCKER in basel.blockers
    assert report.bank_evidence_point_in_time_ready is False
    assert report.readiness_promotion_allowed is False
