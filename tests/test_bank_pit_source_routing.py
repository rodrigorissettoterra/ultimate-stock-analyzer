...
def test_bank_model_weight_inventory_excludes_unevidenced_five_year_growth() -> None:
    audit = audit_bank_pit_source_routing()
    ...
    assert abs(audit.timestamped_candidate_or_better_model_weight - 0.375) < 1e-12
    ...
    assert metrics["net_income_cagr_5y"].status == "unresolved"
