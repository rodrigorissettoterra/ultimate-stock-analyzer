...
def test_share_action_ex_session_outside_dataset_window_is_not_materialized() -> None:
    bars = [
        _bar("TEST3", date(2025, 12, 30), 40.0, open_price=39.0, isin="BRTESTACNOR0"),
        _bar("TEST3", date(2026, 1, 2), 10.0, open_price=10.0, isin="BRTESTACNOR0"),
    ]
    payload = {
        "stockDividends": [
            {
    ...
    assert audit.share_conversions[0].action is not None
    assert audit.share_conversions[0].action.ex_date == date(2026, 1, 2)
    assert dataset.share_actions == ()
    assert all(point.trading_date <= END for point in dataset.prices)
