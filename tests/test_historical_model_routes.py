...
def test_route_is_frozen_after_validation() -> None:
    route = _route(point_in_time_eligible=False)
    registry = HistoricalModelRouteRegistry([route])

    with pytest.raises(ValidationError, match="frozen"):
        route.point_in_time_eligible = True
    with pytest.raises(ValidationError, match="frozen"):
        registry.routes()[0].company_id = "cvm:9512"
    ...
    assert decision.blockers == (HISTORICAL_MODEL_ROUTE_NOT_POINT_IN_TIME,)


def test_model_copy_updates_are_forbidden() -> None:
    route = _route(point_in_time_eligible=False)
    with pytest.raises(TypeError, match="construct a new validated route"):
        route.model_copy(update={"point_in_time_eligible": True})


def test_registry_revalidates_unchecked_constructed_instance() -> None:
    data = _route().model_dump(mode="python")
    data["available_from"] = AVAILABLE.replace(tzinfo=None)
    unchecked = HistoricalModelRoute.model_construct(**data)
    with pytest.raises(ValidationError, match="timezone-aware"):
        HistoricalModelRouteRegistry([unchecked])
...
def test_noncanonical_route_identity_is_rejected(company_id: str) -> None:
    with pytest.raises(ValidationError, match="canonical"):
        _route(company_id=company_id)


@pytest.mark.parametrize(
    "company_id",
    ["CVM:19348", " cvm:19348", "cvm:19348 ", "cvm:1٢"],
)
def test_noncanonical_lookup_identity_is_rejected(company_id: str) -> None:
    registry = HistoricalModelRouteRegistry([_route()])
