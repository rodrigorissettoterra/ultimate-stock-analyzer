...
def test_persistence_consumes_exact_bytes_that_passed_checksum_verification(
    tmp_path: Path,
    ...
    original_verify = persistence_module._verified_artifact_bytes

    def _verify_then_replace(
        run_dir_arg: Path,
        artifact: BootstrapArtifact,
    ) -> bytes:
        content = original_verify(run_dir_arg, artifact)
        path = run_dir_arg / artifact.path
