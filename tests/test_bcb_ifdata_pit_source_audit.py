...
def test_invalid_odata_value_shape_raises_type_error() -> None:
    invalid_sample = (202412, "report_1", json.dumps({"value": {}}).encode())

    with pytest.raises(TypeError, match="OData value list"):
        audit_bcb_ifdata_pit_source(
            metadata_content=_metadata(),
            sample_payloads=(invalid_sample,),
            requested_ano_mes=(202412,),
            collected_at=datetime(2026, 9, 2, tzinfo=UTC),
            source_dataset_url=(
                "https://dadosabertos.bcb.gov.br/dataset/"
                "ifdata---dados-selecionados-de-instituies-financeiras"
