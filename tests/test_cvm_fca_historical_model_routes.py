...
def _rules() -> tuple[FCAModelRouteRule, ...]:
    return (
        FCAModelRouteRule(sector_activity="Bancos", model_id="banks"),
        FCAModelRouteRule(sector_activity="Petróleo e Gás", model_id="commodities"),
        FCAModelRouteRule(sector_activity="Extração Mineral", model_id="commodities"),
    )
...
def _mapping(
    *,
    ...
    mapping_rule_version: str = "fca-sector-activity-v0.1",
) -> FCAHistoricalModelRouteMapping:
...
def test_explicit_fca_mapping_aligns_with_current_sector_registry() -> None:
    mapping = _mapping()
    mapping.validate_against_registry(_registry())


def test_materializes_explicit_routes_without_default_fallback() -> None:
    ...
    assert all(len(route.evidence_sha256) == 64 for route in result.routes)
    assert all("fca-sector-activity-v0.1" in route.mapping_rule_version for route in result.routes)
...
def test_mapping_cannot_encode_sector_registry_fallback_as_explicit_route() -> None:
    rules = (FCAModelRouteRule(sector_activity="Unknown Sector", model_id="general_corporate"),)
    with pytest.raises(ValueError, match="diverges from sector registry"):
        _mapping(mappings=rules).validate_against_registry(_registry())


def test_mapping_rejects_duplicate_sector_labels() -> None:
    duplicate_rules = (
        FCAModelRouteRule(sector_activity="Bancos", model_id="banks"),
        FCAModelRouteRule(sector_activity="Bancos", model_id="commodities"),
    )
...
def test_mapping_rule_changes_route_evidence_hash() -> None:
    ...
    second = materialize_fca_historical_model_routes(
        ledgers=[ledger],
        mapping=_mapping(mapping_rule_version="fca-sector-activity-v0.2"),
        sector_registry=_registry(),
    )
