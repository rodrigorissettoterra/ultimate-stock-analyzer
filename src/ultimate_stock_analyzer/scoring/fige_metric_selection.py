...
def evaluate_fige_metric_selection(
    ...
    for rule in contract.metrics:
        ...
        saturation_evaluated = rule.source == "annual_metric" and len(known) >= 2
        evidence.append(
            FigeMetricEvidence(
                name=rule.name,
                source=rule.source,
                ...
                empirically_saturated=(
                    saturation_evaluated and len(unique_values) <= 1
                ),
    ...
    saturated = tuple(
        ...
        if item.saturation_evaluated and item.empirically_saturated
    )
    if saturated:
        warnings.append("EMPIRICALLY_SATURATED_ANNUAL_METRICS:" + ",".join(sorted(saturated)))
