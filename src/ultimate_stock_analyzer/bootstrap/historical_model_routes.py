...
_DEFAULT_MAPPING_PATH = Path("config/backtesting/fca_model_routes_v0.2.yml")
_DEFAULT_SECTOR_REGISTRY_PATH = Path("config/scoring/sector_registry_v0.6.yml")
...
def persist_historical_model_routes(
    run_dir: str | Path,
    *,
    mapping_path: str | Path = _DEFAULT_MAPPING_PATH,
    sector_registry_path: str | Path = _DEFAULT_SECTOR_REGISTRY_PATH,
) -> PublicDataBootstrapManifest:
    ...
    *,
    ...
    for route in routes:
        ...
        if not route.source_document.startswith(document_prefix) or ":Versao=" not in (
            ...
            raise ValueError(
                f"historical model-route source document is not bound to the "
