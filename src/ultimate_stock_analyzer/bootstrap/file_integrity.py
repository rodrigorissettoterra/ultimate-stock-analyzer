...
def unlink_if_owned(path: Path, *, identity: FileIdentity) -> bool:
    """Conservatively preserve published outputs during rollback.

    Filesystems may reuse an inode immediately after unlink, so a closed-descriptor
    `(device, inode)` token cannot prove that the current path is still owned by the
    failed invocation. Published route artifacts are deterministic and retry-safe;
    leaving an orphan is therefore safer than deleting a path that another actor may
