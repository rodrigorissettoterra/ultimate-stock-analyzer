...
def convert_b3_cash_distribution(
    *,
    ...
    event_ticker = _optional_identity(payment.ticker)
    ...
    if event_ticker is not None and event_ticker != normalized_ticker:
        return _blocked_cash(
            ticker=normalized_ticker,
            payment=payment,
    ...
    if event_ticker is None and event_isin is None:
        return _blocked_cash(
            ticker=normalized_ticker,
            payment=payment,
    ...
    if event_ticker is None and event_isin is not None and not bar_isins:
        return _blocked_cash(
            ticker=normalized_ticker,
            payment=payment,
            status=CASH_DISTRIBUTION_IDENTITY_INSUFFICIENT,
    ...
    if event_isin is not None and bar_isins and event_isin not in bar_isins:
        return _blocked_cash(
            ticker=normalized_ticker,
            payment=payment,
...
def audit_b3_event_aware_coverage(
    *,
    ...
    for event in contract.stock_actions:
        ...
        elif not _event_in_window(event.last_date_prior, start_date, end_date):
            continue
        else:
            relevant_stock.append(event)
    ...
    for payment in payments:
        ...
        if scope == _OTHER:
            continue
        if _event_in_window(payment.ex_date, start_date, end_date):
...
def _event_scope(
    *,
    ...
    if normalized_asset is not None:
        return _TARGET if normalized_asset == ticker else _OTHER
    normalized_isin = _optional_identity(isin)
    if normalized_isin is not None:
        if not target_isins:
            return _AMBIGUOUS
        return _TARGET if normalized_isin in target_isins else _OTHER
    return _AMBIGUOUS
