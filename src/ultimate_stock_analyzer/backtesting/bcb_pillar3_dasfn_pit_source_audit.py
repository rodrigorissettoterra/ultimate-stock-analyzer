...
_REQUIRED_CATALOG_FIELDS = ("Api", "Versao", "Recurso", "Url")
_REVISION_TOKENS = (
    "asof",
    "atualiz",
    "publica",
    ...
    *,
    ...
    if query_available:
        ...
        if rows is None:
            ...
            if not _all_rows_match_known_contract(rows):
                blockers.add(PILLAR3_DASFN_CATALOG_CONTRACT_UNUSABLE)
            else:
...
def _all_rows_match_known_contract(rows: tuple[dict[str, Any], ...]) -> bool:
    for row in rows:
        values = {
