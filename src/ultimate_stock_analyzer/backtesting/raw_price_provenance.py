...
def raw_price_fingerprint(bars: Iterable[PriceBar]) -> str:
    """Fingerprint the exact normalized raw COTAHIST bars consumed by M15."""
...
def bootstrap_raw_price_fingerprint(
    ...
    *,
    ...
    """Fingerprint one exact bootstrap universe/window when price coverage is complete.

    Missing requested tickers deliberately return ``None``. The historical readiness audit can
    then emit its normal ``PRICE_HISTORY_INCOMPLETE`` evidence plus a provenance mismatch instead
    of aborting before the JSON report is written.
    """
