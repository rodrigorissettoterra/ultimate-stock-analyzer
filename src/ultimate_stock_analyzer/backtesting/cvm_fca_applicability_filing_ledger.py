...
FCA_APPLICABILITY_ISSUER_IDENTITY_MISMATCH = "FCA_APPLICABILITY_ISSUER_IDENTITY_MISMATCH"
FCA_APPLICABILITY_REFERENCE_DATE_MISMATCH = "FCA_APPLICABILITY_REFERENCE_DATE_MISMATCH"
...
class FCAApplicabilityFiling:
    ...
    issuer_identity_match: bool = True
    reference_date_match: bool = True
...
class FCAApplicabilityFilingLedger:
    ...
    applicability_detail_codes_observed: tuple[int, ...]
    missing_applicability_detail_codes: tuple[int, ...]
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        payload["applicability_detail_codes_observed"] = list(
            self.applicability_detail_codes_observed
        )
        payload["missing_applicability_detail_codes"] = list(
            self.missing_applicability_detail_codes
        )
...
def build_fca_applicability_filing_ledger(
    *,
    ...
    """Bind FCA applicability rows to exact same-issuer, same-period root filings."""
    ...
    selected_detail_rows: list[dict[str, str]] = []
    detail_codes_observed: set[int] = set()
    for detail in detail_rows:
        code = _int(detail.get("Codigo_CVM"))
        if code in requested_set:
            selected_detail_rows.append(detail)
            detail_codes_observed.add(code)

    missing_detail_codes = tuple(sorted(requested_set - detail_codes_observed))
    filings: list[FCAApplicabilityFiling] = []
    blockers: set[str] = set()
    if missing_detail_codes:
        blockers.add(FCA_APPLICABILITY_DETAIL_NOT_FOUND)

    for detail in selected_detail_rows:
        code = _int(detail.get("Codigo_CVM"))
        if code is None:
            blockers.add(FCA_APPLICABILITY_ISSUER_IDENTITY_MISMATCH)
            continue
        ...
        detail_cnpj = _digits(detail.get("CNPJ_Companhia"))
        root_cnpj = _digits(root.get("CNPJ_CIA"))
        root_code = _int(root.get("CD_CVM"))
        if not detail_cnpj or root_code != code or root_cnpj != detail_cnpj:
            blockers.add(FCA_APPLICABILITY_ISSUER_IDENTITY_MISMATCH)
            continue

        reference_date = _date(detail.get("Data_Referencia"))
        root_reference_date = _date(root.get("DT_REFER"))
        if reference_date is None:
            raise ValueError("FCA applicability detail row has no Data_Referencia")
        if root_reference_date is None or root_reference_date != reference_date:
            blockers.add(FCA_APPLICABILITY_REFERENCE_DATE_MISMATCH)
            continue
    ...
    return FCAApplicabilityFilingLedger(
        ...
        applicability_detail_count=len(selected_detail_rows),
        applicability_detail_codes_observed=tuple(sorted(detail_codes_observed)),
        missing_applicability_detail_codes=missing_detail_codes,
        filings=tuple(
            sorted(
                filings,
                key=lambda item: (
