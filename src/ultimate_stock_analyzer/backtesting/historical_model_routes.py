...
def _canonical_company_id(value: str) -> str:
    if not _CANONICAL_COMPANY_ID.fullmatch(value):
        raise ValueError("historical model-route company_id must be exact canonical cvm:<CD_CVM>")
    return value
...
class HistoricalModelRoute(BaseModel):
    ...
    def model_copy(
        ...
        *,
        ...
        """Preserve validated state; changed routes must be constructed and validated anew."""
        if update:
            raise TypeError(
                "HistoricalModelRoute forbids model_copy(update=...); "
                "construct a new validated route instead"
            )
...
class HistoricalModelRouteRegistry:
    ...
    def __init__(
        ...
        for route in routes:
            # Never trust instance construction history. model_construct() and third-party
            # deserializers can bypass Pydantic field validation, so cross the registry
            # boundary through a fresh validated model built from plain field data.
            validated = HistoricalModelRoute.model_validate(route.model_dump(mode="python"))
            existing = self._routes.get(validated.key)
            if existing is not None and existing != validated:
                raise ValueError(
                    "conflicting historical model routes for "
                    f"{validated.company_id}/{validated.fiscal_year}"
                )
            self._routes[validated.key] = validated
