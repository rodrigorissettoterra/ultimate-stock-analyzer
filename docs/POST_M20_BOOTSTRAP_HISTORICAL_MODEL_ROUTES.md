...
After a route staging file is moved into place, the invocation records the
published file identity. Rollback removes it only while the same inode is still
present; a replacement created by another actor is never unlinked as if it were
