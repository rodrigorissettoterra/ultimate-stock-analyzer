...
The event-aware absolute return must be smaller for every converted event. The smoke also decomposes
that close-to-close event-aware return into:

- the residual mechanical mismatch from COM close to EX open; and
- the real intraday return from EX open to EX close.

Only the mechanical-open residual is bounded by the 15% empirical tolerance. A large real intraday
move is retained rather than censored. The decomposition must reconstruct the M15 close-based return
to numerical precision. This validates the economic effect without modifying historical price bars.
