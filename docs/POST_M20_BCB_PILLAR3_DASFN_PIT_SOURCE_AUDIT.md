...
1. is the official DASFN catalog endpoint queryable now?
2. does a successful catalog response expose the documented Pillar 3 v1/v2
   resource families?
...
The v3 audit therefore performs two bounded probes:
...
If the Pillar 3 query returns usable OData JSON, the audit may record:
...
Those selectors identify a report reference period. They are not historical
as-of/vintage selectors. Likewise, observing a catalog response at collection
time does not prove when an institution-hosted payload was published or revised.
