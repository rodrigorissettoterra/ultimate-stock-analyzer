...
CNPJ is a cross-source validation field, not an identity key. For this audit only, both B3 representations are reduced to digits and left-padded to the canonical 14-digit display width before equality comparison. This handles official B3 responses that omit a leading zero without changing the repository-wide company identity contract.
