...
def main() -> None:
    ...
    for company, events in supported_events.items():
        ...
        for event in events:
            ...
            if raw_return is None or event_aware_return is None:
                continue
            mechanical_open_residual = (
                conversion.action.ratio_new_per_old * evidence.post_open / evidence.pre_close - 1.0
            )
            ex_session_intraday_return = evidence.post_close / evidence.post_open - 1.0
            reconstructed_event_aware_return = (
                (1.0 + mechanical_open_residual) * (1.0 + ex_session_intraday_return) - 1.0
            )
            economic_regressions.append(
                {
                    ...
                    "mechanical_open_residual": mechanical_open_residual,
                    "absolute_mechanical_open_residual": abs(mechanical_open_residual),
                    "ex_session_intraday_return": ex_session_intraday_return,
                    "reconstructed_event_aware_return": reconstructed_event_aware_return,
                    "decomposition_error": abs(
                        reconstructed_event_aware_return - event_aware_return
                    ),
                }
            )
    ...
    report = {
        ...
        "max_absolute_mechanical_open_residual": max(
            (item["absolute_mechanical_open_residual"] for item in economic_regressions),
            default=None,
        ),
        "max_decomposition_error": max(
            (item["decomposition_error"] for item in economic_regressions),
            default=None,
        ),
